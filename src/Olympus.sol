...
contract Olympus is RewardDistributor, ERC721Holder {
    ...
    constructor(
        address _merakiToken,
        uint256 _founderTokenCap,
        ERC20 rewardToken
    ...
    function adjustFounderInfo(
        ...
        require(
            _founders.length == _balances.length,
            "Meraki: Inputs mismatched length"
        );
        ...
        for (uint256 i = 0; i < founderList.length; i++) {
            require(
                founderList[i] != address(0),
                "Meraki: address(0) cannnot be a founder"
            );
            founderBalance[founderList[i]] = _balances[i];
            total += _balances[i];
        }
        require(
            total <= founderDepositCap,
            "Meraki: _balances sum excedes founderDepositCap"
        );
    ...
    /****************************external mutative *************************************/
    function stake(uint256[] memory _ids) external whenNotPaused {
        require(_ids.length > 0, "Meraki: _ids is empty");
        //update reward info
        _updateRewards(msg.sender);
        for (uint256 i = 0; i < _ids.length; i++) {
            MerakiToken.safeTransferFrom(
                msg.sender,
                address(this),
                _ids[i],
                ""
    ...
    function DAOVotingPower(address _user) public view returns (uint256) {
        return
            balanceOf(_user) +
            ((founderBalance[_user] * (MAX_SUPPLY - founderDepositCap)) /
                founderDepositCap); //account for founders reduced voting power
    }

    function usersNFTs(address _user)
        public
        view
